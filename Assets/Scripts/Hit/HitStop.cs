...
public class HitStop : MonoBehaviour
{
    ...
    /// <summary>
    /// Slow motion dramático para o parry: reduz timeScale para <paramref name="slowScale"/>
    /// por <paramref name="holdDuration"/> segundos reais, depois sobe suavemente de volta
    /// a 1 ao longo de <paramref name="rampUpDuration"/> segundos reais.
    ///
    /// Diferente do DoHitStop (timeScale=0), o slow motion mantém animações e física
    /// visíveis em câmera lenta — o jogador vê o inimigo recuando em slow-mo antes
    /// do controle ser devolvido. Essa é a sensação do parry em Blasphemous.
    /// </summary>
    public void DoSlowMotion(float slowScale = 0.05f, float holdDuration = 0.10f, float rampUpDuration = 0.18f)
    {
        slowScale = Mathf.Clamp01(slowScale);

        // Se há um freeze total ativo e mais longo, não interrompe.
        if (_routine != null && _currentDuration > holdDuration + rampUpDuration) return;
        if (_routine != null) StopCoroutine(_routine);

        _currentDuration = holdDuration + rampUpDuration;
        _routine = StartCoroutine(SlowMotionRoutine(slowScale, holdDuration, rampUpDuration));
    }
    ...
    private System.Collections.IEnumerator SlowMotionRoutine(float slowScale, float holdDuration, float rampUpDuration)
    {
        ...
        // Ramp up suave de slowScale → 1
        float elapsed = 0f;
        while (elapsed < rampUpDuration)
        {
            elapsed += Time.unscaledDeltaTime;
            Time.timeScale = Mathf.Lerp(slowScale, 1f, elapsed / rampUpDuration);
            yield return null;
        }
