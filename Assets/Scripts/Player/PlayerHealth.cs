...
public class PlayerHealth : MonoBehaviour
{
    ...
    void Start()
    {
        OnDeath += () => GameOverMenu.Instance?.Show();
    }
    ...
    public void TakeDamage(int damage)
    {
        ...
        Debug.Log($"[PlayerHealth] TakeDamage chamado! parry={parryWindowOpen} defend={isDefending} invincible={isInvincible}");

        if (parryWindowOpen && parryCooldownTimer <= 0f) { ExecuteParry(); return; }
        if (isDefending) { Debug.Log("[PlayerHealth] Bloqueou!"); return; }
        if (isInvincible) { Debug.Log("[PlayerHealth] Invencível!"); return; }
