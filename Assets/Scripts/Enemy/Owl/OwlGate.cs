...
public class OwlGate : MonoBehaviour
{
    ...
    {
        if (owlsRequired <= 0)
            Debug.LogWarning("[OwlGate] owlsRequired está em 0 — o portão vai abrir imediatamente!");

        if (gateObjects == null || gateObjects.Length == 0)
            Debug.LogWarning("[OwlGate] Nenhum Gate Object atribuído — o portão não tem visual/colisão para desativar.");
