...
public class OwlEnemy : MonoBehaviour, IDamageable, IStaggerable
{
    ...
    private void DoLeaping()
    {
        ...
        foreach (var col in hits)
        {
            ...
            ph.TakeDamage(leapDamage, transform.position);
    ...
    private IEnumerator TelegraphRoutine()
    {
        ...
        if (_playerTransform == null || PlayerDistance() < telegraphCancelRange)
        {
    ...
    private IEnumerator LeapRoutine()
    {
        ...
        yield return new WaitUntil(() => IsGrounded() || _state == State.Stagger || _state == State.Dead);
