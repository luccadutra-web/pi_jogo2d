...
public class MeleeWeapon : MonoBehaviour
{
    ...
    [SerializeField] private float finisherKnockback   = 18f;
    ...
    private bool  lightBuffered  = false;
    private bool  heavyBuffered  = false;
    ...
    {
        // Buffers
        if (lightBufferTimer > 0f)
        {
            lightBufferTimer -= Time.deltaTime;
            if (lightBufferTimer <= 0f) 
            {
                lightBuffered = false;
            }        
        ...
        if (heavyBufferTimer > 0f)
        {
            heavyBufferTimer -= Time.deltaTime;
            if (heavyBufferTimer <= 0f) 
            {
                heavyBuffered = false;
            }
        ...
        if (comboWindowTimer > 0f)
        {
            ...
            {
                Debug.Log("[Melee] Janela de combo fechou — combo resetado.");
    ...
    private IEnumerator AttackRoutine(AttackType type)
    {
        ...
        float startup    = type == AttackType.Light    ? lightStartup
                         : type == AttackType.Heavy    ? heavyStartup
                                                       : finisherStartup;
        float activeTime = type == AttackType.Light    ? lightActiveTime
                         : type == AttackType.Heavy    ? heavyActiveTime
                                                       : finisherActiveTime;
        float recovery   = type == AttackType.Light    ? lightRecovery
                         : type == AttackType.Heavy    ? heavyRecovery
                                                       : finisherRecovery;

       
        int trigger = type == AttackType.Light    ? AnimLight
                    : type == AttackType.Heavy    ? AnimHeavy
                                                  : AnimFinisher;
    ...
    {
        float range   = type == AttackType.Light    ? lightAttackRange
                      : type == AttackType.Heavy    ? heavyAttackRange
                                                    : finisherAttackRange;
        int   damage  = type == AttackType.Light    ? lightDamage
                      : type == AttackType.Heavy    ? heavyDamage
                                                    : finisherDamage;
        float knock   = type == AttackType.Light    ? lightKnockback
                      : type == AttackType.Heavy    ? heavyKnockback
                                                    : finisherKnockback;
        float hitStop = type == AttackType.Light    ? lightHitStop
                      : type == AttackType.Heavy    ? heavyHitStop
                                                    : finisherHitStop;
